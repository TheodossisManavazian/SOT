...
# Welcome window is the first window we see when we start the GUI
# Handles all of our programs functions.
class WelcomeWindow(Screen):
    ...
    def anlz(self):
        ...
        try:
            ...
            self.ids.name.text = t.description
            self.ids.price.text = f"${t.lastPrice}"
            ...
            self.ids.RSI.text = str(t.RSI)
            self.ids.MACD.text = f"Val: {t.MACDVal} -- Sig: {t.MACDSignal}"
            self.ids.MA200.text = str(t.movingAvg200)
            self.ids.MA50.text = str(t.movingAvg50)
            self.ids.EMA15.text = str(t.EMA)
            self.ids.Resistance.text = str(f"{t.Resistance},    {t.percentOffResist}%")
            self.ids.Support.text = str(f"{t.Support},    {t.percentOffSupp}%")
            if t.score >= 0:
                self.ids.Score.color = (0, 1, 0, .8)
            else:
                self.ids.Score.color = (1, 0, 0, .8)
            self.ids.Score.text = str(t.score)
            ...
            # =====================
            close_prices = t.cleanPriceHistory['close']
    ...
    def plot(self):
        ...
        symbol = self.ids.symbol.text
        try:
            t = Ticker(client, symbol.upper(), selection)
            t.show_plot()
        except Exception as e:
